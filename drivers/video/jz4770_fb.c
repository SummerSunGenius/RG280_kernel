...
void spi_writ_bit16(unsigned short reg, unsigned short val)
{ 
	unsigned char no;
...
void spi_send_value(unsigned short reg, unsigned short val)
{ 
	unsigned char no;
	unsigned short value;
...
static void jzfb_set_panel_mode(struct jzfb *jzfb)
{
	...
	writew(LCD_RGBC_EVEN_GBR << LCD_RGBC_EVENRGB_BIT,jzfb->base + LCD_RGBC);
